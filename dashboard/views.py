...
@login_required
def index(request):
  score = ZitesoScore.objects.filter(user=request.user).first()
...
def BusinessCreditReports(request):
  ...
  context = {}
  context['reports'] = reports
  score = ZitesoBusinessIntelligenceScore.objects.filter(user=request.user).first()
  context['score'] = score
  ...
  return render(request, 'dashboard/business-credit-report.html',context)
...
def YourCompanyInformation(request):
  ...
  context = {}
  context['company'] = company
  return render(request, 'dashboard/view-company-information.html',context)
...
def DisputeView(request):
  ...
  tradelines = YourCurrentTradelines.objects.filter(user=request.user)
  context['tradelines'] = tradelines
  return render(request, 'dashboard/dispute-business-report.html',context)
...
def UpdateCompanyInformation(request):
  ...
  context['company'] = company
  return render(request, 'dashboard/update-company-information.html',context)
...
def CurrentBusinessTradelines(request):
  ...
  tradelines = YourCurrentTradelines.objects.filter(user=request.user)
  context['tradelines'] = tradelines
  return render(request, 'dashboard/view-current-business-tradelines.html',context)
